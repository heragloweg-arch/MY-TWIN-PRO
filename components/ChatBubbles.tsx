...
// استيراد الأفاتار الحي
const DEFAULT_ICON = require('../assets/icon.png');
import { getEmotionColor } from '../utils/theme';
import { apiGet } from '../lib/httpClient';

// ألوان المشاعر المحدّثة
const EMOTION_COLORS: Record<string, string> = {
  joy: '#FFD700', sadness: '#4A90E2', anger: '#FF3B30', fear: '#9C27B0',
  love: '#E91E63', surprise: '#FF9800', neutral: '#7C3AED',
};

// مكون الأفاتار الحي (مُحدّث)
const TwinAvatar = memo(({ userId, emotion, size = 36 }: { userId?: string; emotion?: string; size?: number }) => {
  const [avatarUrl, setAvatarUrl] = useState<string | null>(null);
  const borderColor = emotion ? (EMOTION_COLORS[emotion] || '#7C3AED') : '#7C3AED';
  ...
  return (
    <View style={[av.wrap, { borderColor }]}>
      <Image
        source={avatarUrl ? { uri: avatarUrl } : DEFAULT_ICON}
...
// فقاعة التوأم المطوّرة – تصميم كوني مع أفاتار حي واسم المزوّد
export const TwinBubble = memo(({ item, isDark, isRTL, isLast, onCopy, onRetry, onRegenerate, onLike, onDislike, provider, lang, userId }: any) => {
  const fadeAnim = useRef(new Animated.Value(0)).current;
  useEffect(() => {
  ...
  return (
    <Animated.View style={[styles.twinRow, { opacity: fadeAnim }]}>
      {/* الأفاتار الحي بدلاً من الأيقونة الثابتة */}
      <View style={styles.twinAvatarContainer}>
        <TwinAvatar userId={userId} emotion={emotion} size={38} />
        <View style={{ flex: 1 }}>
      ...
      <View style={[styles.twinCard, { backgroundColor: c.twinBg, borderColor: c.border }]}>
        <View style={[styles.twinMeta, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
          <View style={[styles.providerBadge, { backgroundColor: c.codeBg, borderLeftColor: emotionColor, borderLeftWidth: 2 }]}>
            ...
            <Text style={[styles.providerText, { color: c.subtext }]}>
              {lang === 'ar' ? prov.ar : prov.en}
            </Text>
          </View>
