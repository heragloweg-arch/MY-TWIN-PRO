...
const T = {
  ar: {
    title: 'باقات الوعي', subtitle: 'اختر عمق وعي توأمك',
    current: 'مفعّلة حالياً', popular: 'الأكثر شيوعاً', bestValue: 'أفضل قيمة',
  ...
  en: {
    title: 'Consciousness Plans', subtitle: 'Choose your Twin\'s depth',
    current: 'Currently Active', popular: 'Most Popular', bestValue: 'Best Value',
...
export default function SubscriptionScreen() {
  const { tier, lang, twinName } = useTwinStore();
  const isAr = lang === 'ar';
  ...
  const colors = {
    ...
    accent: '#7C3AED', border: isDark ? '#2D1B4D' : '#E8E8E3', success: '#10B981',
  };

  const handleSubscribe = async (plan: Plan) => {
    if (plan.id === 'free' || plan.id === tier) return;
    setLoading(plan.id);
    const url = `${LANDING_PAGE_URL}?plan=${plan.id}&user=${twinName || 'user'}`;
    try {
      try {
        const iapService = require('../lib/iapService');
        await iapService.initialize();
        const result = await iapService.purchaseSubscription(plan.id);
        if (result?.success) { Alert.alert('✅', isAr ? 'تم الاشتراك' : 'Subscribed'); return; }
      } catch (e) {}
      await Linking.openURL(url);
    } catch (e: any) {
      if (e?.code !== 'E_USER_CANCELLED') await Linking.openURL(url);
  ...
  const handleRestore = async () => {
    try {
      ...
      Alert.alert('✅', isAr ? 'تم استعادة الاشتراك' : 'Subscription restored');
    } catch (e) { Linking.openURL(LANDING_PAGE_URL); }
  };
    ...
    <View style={[st.root, { backgroundColor: colors.bg }]}>
      <View style={[st.header, { borderBottomColor: colors.border }]}>
        <TouchableOpacity onPress={() => router.back()} style={st.backBtn}>
          <ArrowRight size={24} stroke={colors.text} style={{ transform: [{ rotate: isAr ? '0deg' : '180deg' }] }} />
        </TouchableOpacity>
      ...
      <ScrollView style={st.scroll} contentContainerStyle={st.content} showsVerticalScrollIndicator={false}>
        <View style={st.heroSection}>
          ...
          <Text style={[st.heroSub, { color: colors.subtext }]}>
            {isAr ? `كل باقة تفتح طبقة جديدة من الوعي لـ ${twinName || 'توأمك'}` : `Each plan unlocks a new layer of consciousness for ${twinName || 'your Twin'}`}
          </Text>
        </View>
        {PLANS.map((plan) => {
          const isCurrent = tier === plan.id;
          const PlanIcon = plan.id === 'free' ? Sparkles : plan.id === 'yearly' ? Crown : plan.id === 'pro' ? Star : plan.id === 'premium' ? Brain : Zap;
          return (
            <View key={plan.id} style={[st.planCard, { backgroundColor: colors.card, borderColor: isCurrent ? plan.color : colors.border, borderWidth: isCurrent ? 2 : 1 }, plan.highlight && { borderColor: plan.color, borderWidth: 2 }, plan.popular && { borderColor: plan.color }]}>
              {plan.popular && (<View style={[st.popularBadge, { backgroundColor: plan.color }]}><Star size={12} stroke="#FFF" /><Text style={st.badgeText}>{t.popular}</Text></View>)}
              {plan.highlight && (<View style={[st.popularBadge, { backgroundColor: plan.color }]}><Crown size={12} stroke="#FFF" /><Text style={st.badgeText}>{t.bestValue}</Text></View>)}
              {isCurrent && (<View style={[st.popularBadge, { backgroundColor: colors.success }]}><CheckCircle2 size={12} stroke="#FFF" /><Text style={st.badgeText}>{t.current}</Text></View>)}
              <View style={st.planHeader}>
                <View style={[st.planIconWrap, { backgroundColor: plan.glowColor }]}><PlanIcon size={28} stroke={plan.color} /></View>
                <View style={{ flex: 1, marginLeft: 14 }}>
                  <Text style={[st.planName, { color: colors.text }]}>{plan.name}</Text>
                  <Text style={[st.planTagline, { color: plan.color }]}>{isAr ? plan.tagline_ar : plan.tagline_en}</Text>
                </View>
              </View>
              <View style={st.priceRow}><Text style={[st.price, { color: colors.text }]}>{plan.price}</Text><Text style={[st.period, { color: colors.subtext }]}>{plan.period}</Text></View>
              {plan.billingNote ? <Text style={[st.billingNote, { color: '#10B981' }]}>{plan.billingNote}</Text> : null}
              <View style={st.consciousnessRow}>
                <Brain size={14} stroke={plan.color} />
                <Text style={[st.consciousnessLabel, { color: colors.subtext }]}>{t.layers}: {plan.consciousnessLayers}/5</Text>
                <View style={st.consciousnessBar}>{[1,2,3,4,5].map(i => (<View key={i} style={[st.consciousnessSeg, { backgroundColor: i <= plan.consciousnessLayers ? plan.color : colors.border }]} />))}</View>
              </View>
              <View style={st.featuresList}>
                {plan.features.map((f, i) => { const Icon = f.icon; return (
                  <View key={i} style={st.featureRow}><Icon size={15} stroke={f.available ? '#10B981' : '#94A3B8'} /><Text style={[st.featureText, { color: f.available ? colors.subtext : '#94A3B8' }]}>{isAr ? f.text_ar : f.text_en}</Text></View>
                );})}
              </View>
              <TouchableOpacity style={[st.subscribeBtn, { backgroundColor: isCurrent ? colors.success : plan.color }]} onPress={() => handleSubscribe(plan)} activeOpacity={0.85} disabled={isCurrent || loading === plan.id}>
                {loading === plan.id ? <ActivityIndicator color="#FFF" /> : <Text style={st.subscribeBtnText}>{isCurrent ? (isAr ? 'مفعّلة ✓' : 'Active ✓') : plan.id === 'free' ? (isAr ? 'الحالية' : 'Current') : t.subscribe}</Text>}
                {!isCurrent && plan.id !== 'free' && <ArrowRight size={18} stroke="#FFF" style={{ marginLeft: 6 }} />}
              </TouchableOpacity>
            </View>
          );
        })}
        <TouchableOpacity style={st.restoreBtn} onPress={handleRestore}><Text style={[st.restoreText, { color: colors.accent }]}>{t.restore}</Text></TouchableOpacity>
        <TouchableOpacity style={st.manageBtn} onPress={handleManage}><Text style={[st.manageText, { color: colors.accent }]}>{t.manage}</Text></TouchableOpacity>
...
const st = StyleSheet.create({
  root: { flex: 1 }, header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 14, borderBottomWidth: 0.5 },
  backBtn: { width: 40, height: 40, justifyContent: 'center', alignItems: 'flex-start' }, headerTitle: { fontSize: 18, fontWeight: '700' },
  scroll: { flex: 1 }, content: { paddingBottom: 50, paddingHorizontal: 16 },
  heroSection: { alignItems: 'center', paddingVertical: 24 }, heroIcon: { width: 72, height: 72, borderRadius: 22, justifyContent: 'center', alignItems: 'center', marginBottom: 16 },
  heroTitle: { fontSize: 24, fontWeight: '800', textAlign: 'center', marginBottom: 8 }, heroSub: { fontSize: 14, textAlign: 'center', lineHeight: 22 },
  planCard: { borderRadius: 24, padding: 20, marginBottom: 16, position: 'relative' },
  popularBadge: { flexDirection: 'row', alignItems: 'center', alignSelf: 'flex-start', gap: 4, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8, marginBottom: 12 },
  badgeText: { color: '#FFF', fontSize: 11, fontWeight: '700' },
  planHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 16 }, planIconWrap: { width: 52, height: 52, borderRadius: 16, justifyContent: 'center', alignItems: 'center' },
  planName: { fontSize: 20, fontWeight: '800', marginBottom: 2 }, planTagline: { fontSize: 12, fontWeight: '600', fontStyle: 'italic' },
  priceRow: { flexDirection: 'row', alignItems: 'baseline', gap: 4, marginBottom: 4 }, price: { fontSize: 34, fontWeight: '800' }, period: { fontSize: 15 },
  billingNote: { fontSize: 12, fontWeight: '600', marginBottom: 16 },
  consciousnessRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 20 }, consciousnessLabel: { fontSize: 12, fontWeight: '600', flex: 1 },
  consciousnessBar: { flexDirection: 'row', gap: 3, flex: 1 }, consciousnessSeg: { flex: 1, height: 6, borderRadius: 3 },
  featuresList: { marginBottom: 8 }, featureRow: { flexDirection: 'row', alignItems: 'flex-start', gap: 8, marginBottom: 6 },
  ...
  restoreBtn: { alignItems: 'center', paddingVertical: 12, marginTop: 4 }, restoreText: { fontSize: 14, fontWeight: '600' },
  manageBtn: { alignItems: 'center', paddingVertical: 8 }, manageText: { fontSize: 13 },
