...
export default function Onboarding() {
  ...
  const freeInfoRef = useRef<TextInput>(null);
  ...
  useEffect(() => {
    ...
    if (answeredCount >= 4 && !avatarStarted.current) {
      avatarStarted.current = true;
      generateBothAvatars(userName.trim() || 'user');
    }
  }, [answers, userName]);

  // 2. التنقل التلقائي بعد انتهاء الكتابة والصوت
  useEffect(() => {
    if (birthReady && typingDone && speakingDone && !autoNavigate) {
  ...
  }, [birthReady, typingDone, speakingDone, autoNavigate]);

  // 3. بدء الصوت والكتابة المتزامنة عند الولادة
  useEffect(() => {
  ...
  const handleBirthConsciousness = async () => {
    ...
    try {
      ...
      await Promise.all([
        ...
        apiPost('/api/memories', {
          user_id: userId,
          type: 'personality_analysis',
          content: analysisText,
          lang,
          twin_name: newTwinName.trim() || (isAr ? 'توأمك' : 'My Twin'),
          metadata: { answers, free_info: freeInfo, twin_gender: newTwinGender },
        }).catch(() => {}),
      ]);
...
const st = StyleSheet.create({
  ...
  inputIconRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 6, marginTop: 14 },
  input: { borderRadius: 14, padding: 14, fontSize: 16, borderWidth: 1 },
  textArea: { borderRadius: 14, padding: 14, fontSize: 16, borderWidth: 1, minHeight: 110 },
  ...
  submitBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 17, borderRadius: 18, gap: 10, width: '100%', marginTop: 8 },
